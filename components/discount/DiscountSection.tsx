...
const DiscountSection = () => {
  const product = products.filter((product) => product.category === "Pants");
  ...
  return (
    <div className="bg-zinc-100 flex py-8 sm:py-12 md:py-16 justify-center items-center flex-col min-h-[70vh] w-full">
      <div className="w-full mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-12 sm:gap-6">
          {product.slice(0, 5).map((board) => (
            <Link
              key={board.id}
              href={`/product/${board.id}`}
              className="group cursor-pointer"
            >
              <div className="border-black border relative w-full min-w-56 min-h-100 h-full transition-transform group-hover:scale-105 overflow-hidden">
                <img
                  src={board.imageUrl}
                  alt={board.name}
